    ...
    public void waitMessage(ChatMainController chatMainController) {
        ...
        Thread thread = new Thread( () -> {
            try { while (true) {
                ...
                if(command == null) {
                    chatMainController.showError("Ошибка серверва", "Получена неверная команда");
                    continue;
                }
                ...
                switch (command.getType()) {
                    case INFO_MESSAGE: {
                        ...
                        Platform.runLater(() -> {
                            chatMainController.appendMessage(formattedMessage);
                        });
                        break;
                    }
                    case ERROR: {
                        ...
                        Platform.runLater(() -> {
                            chatMainController.showError("Server error", errorMessage);
                        });
                        break;
                    }
                    case UPDATE_USERS_LIST: {
                        UpdateUsersListCommandData data = (UpdateUsersListCommandData) command.getData();
                        Platform.runLater(() -> chatMainController.updateUsers(data.getUsers()));
                        break;
                    }
                    default:
                        Platform.runLater(() -> {
                            chatMainController.showError("Unknown command from server!", command.getType().toString());
                        });
                }
